...
def index(request, auth_form=None, user_form=None):
	...
	if request.user.is_authenticated():
		...
		return render(request, 'buddies.html', {'ribbit_form': ribbit_form, 'user': user, 'ribbits': ribbits, 'next_url':'/',})
...
def get_latest(user):
	try:
		return user.rebbit_set.order_by('-id')[0]
	except IndexError:
		return ""
...
def users(request, username="", ribbit_form=None):
	...
	ribbit_form = ribbit_form or RibbitForm()
	return render(request, 'profiles.html', {'obj': obj, 'next_url': '/users/', 'ribbit_form': ribbit_form, 'username': request.user.username, })
